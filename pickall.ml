...
(*
 * Identify the stocks, in sorted order, that currently pass each screen.
 * Applies the "basic" screen if one is defined within the input files.
 *)
...
let main () =
	Arg.parse
		[beg_spec; emax_spec; ties_spec]
		add_file
		"arg(s): [-begin YYYYMMDD] [-max count] [-ties] file1 file2 ...";
	...
	in
	List.iter compile (Compiler.screen_names ());
	...
	let apply snm =
		...
		print_newline ()
	in
	List.iter apply (List.sort compare (Compiler.screen_names ()));
	print_newline ();
	print_endline dir;
	exit 0;;
